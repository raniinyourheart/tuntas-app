...
>
  <div className="container mx-auto px-4 md:px-6 flex justify-between items-center">
    ...
    <div className="hidden md:flex items-center gap-8 text-white font-medium">
      {["Beranda", "Tentang", "Layanan", "Pengaduan"].map((item) => (
        <button
          key={item}
    ...
    {/* MOBILE MENU */}
    <button className="md:hidden text-white text-3xl hover:text-[#d4af37] transition-all duration-300 hover:scale-110">
      ☰
    </button>
  </div>
</nav>
      ...
      <section id="beranda" className="relative text-white pt-32 pb-20 md:py-32 overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-6 text-center relative z-20">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button onClick={onNavigateToForm} className="bg-[#d4af37] hover:bg-[#c4a030] text-[#0a1a3a] font-semibold px-8 py-3 rounded-lg transition-all duration-300 shadow-lg hover:scale-105 hover:shadow-2xl">
              📝 Buat Pengaduan Baru
            </button>
            <button onClick={onNavigateToStatus} className="bg-transparent border-2 border-white hover:bg-white hover:text-[#0a1a3a] font-semibold px-8 py-3 rounded-lg transition-all duration-300 hover:scale-105">
      ...
      <section className="py-20 bg-gray-100">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { icon: "📌", title: "Sarana & Prasarana", desc: "AC, kursi, lab, fasilitas rusak" },
              { icon: "📋", title: "Pelayanan Administrasi", desc: "Kurang nyaman atau lambat" },
              { icon: "⚠️", title: "Perilaku Tidak Sesuai", desc: "Melanggar aturan kampus" },
              { icon: "💡", title: "Saran Perbaikan", desc: "Untuk organisasi & tata kelola" },
            ].map((item, idx) => (
              <div key={idx} className="bg-white p-6 rounded-xl shadow-md hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 cursor-pointer card-hover-glow">
                <div className="text-5xl mb-4 icon-hover">{item.icon}</div>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 max-w-5xl mx-auto">
            {[
              { number: "1", title: "Pilih Status", desc: "Mahasiswa / Dosen / Masyarakat Umum" },
              { number: "2", title: "Tulis Masukan", desc: "Kritik, saran, atau pengaduan" },
              { number: "3", title: "Upload Gambar", desc: "PDF/PNG/JPG (opsional)" },
              { number: "4", title: "Kirim ", desc: "Dapatkan status pengaduan" },
            ].map((step, idx) => (
              <div key={idx} className="text-center group cursor-pointer step-card">
                <div className="w-16 h-16 bg-gradient-to-br from-[#0a1a3a] to-[#0f2a4a] text-[#d4af37] rounded-full flex items-center justify-center text-2xl font-bold mx-auto mb-4 transition-all duration-300 group-hover:scale-110 group-hover:shadow-xl step-number-hover">
                  {step.number}
                </div>
      ...
      <section className="py-20 bg-[#0a1a3a] text-white">
        <div className="container mx-auto px-4 md:px-6 text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: "🔓", text: "Tidak perlu login" },
              { icon: "🙈", text: "Bisa anonim" },
              { icon: "📎", text: "Upload dokumen pendukung" },
              { icon: "📊", text: "Status jelas: diterima -> diproses -> selesai" },
            ].map((item, idx) => (
              <div key={idx} className="bg-[#0f2a4a] p-6 rounded-xl transition-all duration-300 hover:scale-105 cursor-pointer advantage-card">
                <div className="text-4xl mb-3 advantage-icon-hover">{item.icon}</div>
        ...
        <div className="container mx-auto px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-xl shadow-lg text-center hover:scale-105 transition service-card">
              <div className="text-4xl mb-3 service-icon-hover">⏱️</div>
            ...
            <div className="bg-gradient-to-br from-green-50 to-green-100 p-6 rounded-xl shadow-lg text-center hover:scale-105 transition service-card">
              <div className="text-4xl mb-3 service-icon-hover">📁</div>
      ...
      <section id="pengaduan" className="relative py-20 text-white overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-6 text-center relative z-20">
          ...
          <button onClick={onNavigateToForm} className="bg-[#d4af37] hover:bg-[#c4a030] text-[#0a1a3a] font-bold px-10 py-4 rounded-lg transition-all duration-300 shadow-xl text-lg hover:scale-105 cta-button">
            📝 Buat Pengaduan Baru
          </button>
        </div>
      </section>
      ...
      <style>{`
        ...
        .shine-horizontal {
          ...
          color: transparent;
          animation: shineHorizontal 3s linear infinite;
        }
        ...
        .shine-horizontal-delay {
          ...
          background-clip: text;
          color: transparent;
          animation: shineHorizontal 3s linear infinite 0.5s;
        }
        
        .animate-blink { animation: blink 1s step-end infinite; }
        .animate-pulse { animation: pulse 2s ease-in-out infinite; }
        
        /* EFEK GERAK SAAT DISENTUH */
        .card-content-hover {
        ...
        .service-text-hover:hover {
          transform: scale(1.02);
          font-weight: bold;
        }
        
        /* EFEK GLOW KUNING UNTUK CARD */
        .card-hover-glow:hover {
          box-shadow: 0 0 20px rgba(212, 175, 55, 0.4), 0 10px 30px rgba(0, 0, 0, 0.1);
        }
